...
# ----- Mesh from gmsh ----- #
name, comm = "softening", MPI.COMM_WORLD
...
Vη_e = ufl.FiniteElement("Quadrature", mesh.ufl_cell(), degree=quad_degree, quad_scheme="default")
...
Vη = dolfinx.fem.FunctionSpace(mesh, Vη_e)
...
η = dolfinx.fem.Function(Vη, name="Mobilized_Friction")
...
functions = {'displacement': u, 'plastic_strain': εp, 'pressure': p,
             'flux': q, 'temperature': None, 'mobilized_friction': η}
old_functions = {'displacement': u_n, 'plastic_strain': εp_n, 'pressure': p_n,
                 'temperature': None}
init_conditions = {'pressure': p_i, 'temperature': None}
...
kk = 25
load = np.linspace(0.064, 1.75, int(kk+1.0))
...
F2 = ufl.inner(δη, EP.mobilized_friction(plasticity_active))*dx
F3 = ufl.inner(δp,EP.mass_balance(dt,strain_active))*dx
F4 = EP.darcy_flux(dt,δq)*dx
...
problem = dolfiny.snesblockproblem.SNESBlockProblem([F0,F1,F2,F3,F4], [u,εp,η,p,q], prefix=name)
...
results = {'σa': [], 'σr': [], 'εa': [], 'εv': [], 'pp': [], 'η': [], 'εq': [], 'εpq': [], 't': []}
...
for step, factor in enumerate(load):
    ...
    problem.bcs = []
    problem.bcs.append(dolfinx.fem.dirichletbc(load_factor, top_dofs_u, Vu.sub(2)))
    ...
    problem.bcs.append(dolfinx.fem.dirichletbc(no_flow,top_dofs_q))
    problem.bcs.append(dolfinx.fem.dirichletbc(no_flow,base_dofs_q))
    ...
    results['η'].append(dolfiny.expression.assemble(η,dx(omega))/Volume)
    results['εq'].append(dolfiny.expression.assemble(εq,dx(omega))/Volume)
    results['εpq'].append(dolfiny.expression.assemble(εpq,dx(omega))/Volume)
    results['t'].append(time_[step])

    for source, target in zip([u,εp,p], [u_n,εp_n,p_n]):
...
ηf = np.array(results['η'])
εqf = np.array(results['εq'])*1e2
εpqf = np.array(results['εpq'])*1e2
t = np.array(results['t'])/60

dict_ = {'Axial_stress': σa, 'Radial_stress': σr, 'Axial_strain': εa,
         'Volumetric_strain': εv, 'Pore_pressure': pp, 'Mobilized_friction': ηf,
         'Deviator_strain': εqf, 'Plas_deviator_strain': εpqf, 'time': t}
